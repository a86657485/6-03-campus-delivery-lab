...
window.Classroom=(()=>{
 ...
 function save(payload){if(!student)return;pending={sid:student.id,eventId:eventId(),version,payload:JSON.parse(JSON.stringify(payload))};disk();callback(blocked?'conflict':'pending',blocked?'另一设备已有更新，请处理同步冲突':'已保存在本机，正在同步');flush();}

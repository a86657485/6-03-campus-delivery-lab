...
test('classroom identity, authorization, isolation, deduplication and persistence',async t=>{
 ...
 const full={...empty(),stage:'robot',completions:proofs};
 saved=await api('/api/progress',{sid:'one',eventId:'all-complete',version:2,payload:full},a.cookie);assert.equal(saved.status,200);assert.equal(saved.data.points,130);assert.equal(saved.data.awards.length,7);
 saved=await api('/api/progress',{sid:'one',eventId:'all-complete',version:2,payload:full},a.cookie);assert.equal(saved.data.version,3);assert.equal(saved.data.points,130);
 saved=await api('/api/progress',{sid:'one',eventId:'practice-again',version:3,payload:empty()},a.cookie);assert.equal(saved.data.points,130);
 await stop();await start();assert.equal((await api('/api/me',undefined,a.cookie)).data.points,130);
